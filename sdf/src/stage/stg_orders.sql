with initial_dates as (
    -- Initialise with Sunday, Jan 2nd 2022 as the first day
    select *
    , COALESCE(days_since_prior_order, 0) as days_since_prior_order_v2
    -- Randomize the day of the first order
    , case order_number
        when 1 then timestamp '01-02-2022 00:00:00 00:00:00' + (round(random()) * 5 * INTERVAL '1 week') + INTERVAL '1 day' * order_dow + INTERVAL '1 hour' * order_hour_of_day
